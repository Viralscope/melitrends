  ...
  try {
    ...
    if (items.length === 0) {
      return res.status(200).json({
        query: q, site,
        total_results: 0,
        sellers: [], top_products: [], price_analysis: null,
      });
    }
    ...
    for (const item of items) {
      ...
      if (!sellerMap[sellerId]) {
        sellerMap[sellerId] = { id: sellerId, nickname: sellerNickname, total_sold: 0, listings: 0, prices: [] };
      }
    ...
    for (const seller of Object.values(sellerMap)) {
      seller.avg_price = seller.prices.reduce((a, b) => a + b, 0) / seller.prices.length;
      delete seller.prices;
